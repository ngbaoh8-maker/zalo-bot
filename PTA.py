...
class CommandHandler:
    def __init__(self, client):
        ...
        self.reaction_icons = {
            'normal': ['Dca nguyen hoang gia bao💎', 'nguyen hoang gia bao Dzai Bá❄️', 'nguyen hoang gia bao Cte🪼'],
            'admin': ['🔧', '⚙️', '🛠️'],
            'error': ['lỗi ní ơi🗿', 'nhờ nguyen hoang gia bao đi🐧', 'dca nguyen hoang gia bao dz 💎'],
            'cooldown': ['đợi xíu❄️', 'đợi dùm đi🥴', 'thời gian ngắn🕒'],
            'stw': ['tốt✨', 'dca nguyen hoang gia bao💫', 'được r💡'] 
        }
    ...
    def _reply_message_blocking(self, error_message, message_object, thread_id, thread_type, author_id):
        ...
        self.client.replyMessage(Message(text=msg, style=styles),
    ...
    def _send_multiple_reactions_blocking(self, message_object, command_type, thread_id, thread_type):
        icons = random.sample(self.reaction_icons.get(command_type, ['✅']),
    ...
    def _handle_cooldown(self, message, message_object, thread_id, thread_type, author_id):
        ...
        if action == 'set':
            ...
            is_valid_command = command in self.PTA or \
                               command in self.stw_commands
    ...
    def _execute_command(self, command_handler, message_text, message_object, thread_id, thread_type, author_id, command_name):
        try:
            ...
            if e.errno == _errno.ENOSPC:
                self.reply_message_async(
                    "⚠️ Server hết dung lượng đĩa! Bot không thể lưu file.\n"
                    "Vui lòng liên hệ admin để dọn dẹp disk.",
                    message_object, thread_id, thread_type, author_id
                )
            else:
    ...
    def handle_command(self, message, author_id, message_object, thread_id, thread_type):
        ...
        if message_text.startswith(self.current_prefix):
            ...
            if command_part_after_prefix:
                ...
                if self.client.bcmd_handler.is_command_blocked(potential_command_name_base, author_id, thread_id):
                    self._send_multiple_reactions(message_object, 'error', thread_id, thread_type)
                    self.reply_message_async("Bạn đã bị cấm sử dụng lệnh này.", message_object, thread_id, thread_type, author_id)
                    return

                if potential_command_name_base in self.PTA:
        ...
        if found_cmd_name:
            ...
            if command_handler:
                self._send_multiple_reactions(message_object, command_type_reaction, thread_id, thread_type)
        ...
        if message_text.startswith(self.current_prefix):
            help_message = (
                f"Bạn đã dùng sai lệnh‼️ Vui lòng chat ví dụ:\n"
                f"{self.current_prefix}menu\n"
                f"Để biết thêm chi tiết lệnh.💎\n"
                f"Dùng {self.current_prefix}cmd hoặc {self.current_prefix}help để xem lệnh dành cho admin 🗿"
            )
            self._send_multiple_reactions(message_object, 'error', thread_id, thread_type)
