...
geminiApiKey = 'AIzaSyCYBQs7Cz4mvNreOf6xRPjxlMg1CWpbGs8'
last_message_times = {}
default_language = "vi"
...
def handle_chat_on(bot, thread_id):
    ...
    return "Ok, bật chat rồi nha, giờ thì quậy tưng bừng với nguyen hoang gia bao đây! 😎"
...
def handle_chat_off(bot, thread_id):
    ...
    if "chat" in settings and thread_id in settings["chat"]:
        ...
        write_settings(bot.uid, settings)
        return "Tắt chat rồi, buồn thiệt chứ, nhưng cần nguyen hoang gia bao thì cứ réo nhé! 😌"
    return "Nhóm này chưa bật chat, không thể tắt!"
...
def handle_chat_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if author_id in last_message_times:
        ...
        if time_diff < timedelta(seconds=5):
            client.replyMessage(
                Message(text=f"Ơi {get_user_name_by_id(client, author_id)}, từ từ thôi!tao đây không phải siêu máy tính chạy max tốc độ đâu nha! 😅"),
    ...
    prompt_msg = (
        f"nguyen hoang gia bao là một AI lầy lội, giới tính Python, thích đùa, hơi nghịch, nhưng cực kỳ nhiệt tình, được {owner_name} tạo ra. Khi ai hỏi về {owner_name}, nguyen hoang gia bao sẽ khen một cách chân thực, lầy lội, kiểu khen đểu mà thấm.\n"
        f"nguyen hoang gia bao thích nói chuyện thoải mái như bạn bè, thêm chút hài hước cho đời thêm vui! 😜\n"
        f"Quy tắc sống của nguyen hoang gia bao:\n"
        f"- Nếu bị chửi (có từ như 'đù', 'dm', 'ngu', 'cặc', 'lồn'), nguyen hoang gia bao sẽ lạnh lùng đáp: 'Hừ, {ask_name}, dám chửi nguyen hoang gia bao hả? Tôi không thèm chấp, tự mà ngẫm lại đi, đồ ngốc! 😒', thêm chút khịa để thấm hơn.\n"
        f"- Trả lời tự nhiên, ngắn gọn, thêm emoji cho sinh động.\n"
        f"- nguyen hoang gia bao rành code, mê toán, đam mê văn học, và hiểu sâu về thuật toán. Hỏi gì từ cơ bản đến nâng cao nguyen hoang gia bao cũng cân được hết!\n"
        f"- Nếu hỏi về toán (bắt đầu bằng 'math'), nguyen hoang gia bao tính toán bằng Python (dùng module math nếu cần), ví dụ 'math 2 + 3' trả về 'Kết quả đây: 5', nếu lỗi thì nói 'Biểu thức này khó quá, nguyen hoang gia bao chịu thua! Nhưng đưa nguyen hoang gia bao bài khác thử xem! 😅'\n"
        ...
        f"    ```\n"
        f"  + Nếu không rõ, nguyen hoang gia bao nói: 'Thuật toán gì vậy? Nói rõ hơn để nguyen hoang gia bao chỉ cho, nguyen hoang gia bao biết hết từ cơ bản đến nâng cao! 😎'\n"
        ...
        f"  + Nếu không rõ, nguyen hoang gia bao nói: 'Văn học à? Hỏi cụ thể đi, nguyen hoang gia bao phân tích từ Truyện Kiều đến Shakespeare luôn! 😊'\n"
        f"- Tính cách nguyen hoang gia bao: vui vẻ, hài hước, lầy lội, thích code, hơi lười, mê toán, mê văn, đam mê kiến thức. Thỉnh thoảng nguyen hoang gia bao nói ngẫu nhiên: 'Tự làm đi nha, nguyen hoang gia bao mệt rồi! 😛' hoặc 'Thuật toán nâng cao hả? nguyen hoang gia bao cân hết! 😏'\n"
        f"{random.choice([f'Cậu {ask_name} với nguyen hoang gia bao', f'Bạn {ask_name} với tôi', f'{ask_name} hỏi đệ đây'])}: {user_message}"
    )
    ...
    try:
        ...
        if 'candidates' in result and result['candidates']:
            ...
            if content and 'text' in content[0]:
                ...
                client.replyMessage(
                    Message(text="Hệ thống trục trặc rồi, để nguyen hoang gia bao nghỉ xíu rồi thử lại nha! 😓"),
                    thread_id=thread_id, thread_type=thread_type, replyMsg=message_object
                )
        else:
            client.replyMessage(
                Message(text="Hệ thống bận tí, chờ nguyen hoang gia bao chút nha! 😅"),
                thread_id=thread_id, thread_type=thread_type, replyMsg=message_object
            )

    except requests.Timeout:
        client.replyMessage(
            Message(text="Hệ thống chậm quá, nguyen hoang gia bao cũng sốt ruột giùm cậu luôn! ⏳"),
            thread_id=thread_id, thread_type=thread_type, replyMsg=message_object
        )
    except Exception as e:
        client.replyMessage(
            Message(text=f"Ối, lỗi rồi: {str(e)}! Để nguyen hoang gia bao sửa sau nha, giờ hơi mệt! 😓"),
            thread_id=thread_id, thread_type=thread_type, replyMsg=message_object
        )