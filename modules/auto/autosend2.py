...
time_messages = {
    ...
    "21:15": ["Ngủ ngon nha, mai có nhiều điều mới! 🌃", "music"],
...
def autosend_all_on(client):
    try:
        groups = []
        threads = client.fetchThreadList()
        for t in threads:
            if t.type == ThreadType.GROUP:
                groups.append(str(t.id))

        # Lưu vào file JSON
        os.makedirs("modules/cache", exist_ok=True)
        with open("modules/cache/sendtask_autosend.json", "w") as f:
            json.dump({"groups": groups}, f)

        return f"✅ Đã bật autosend cho {len(groups)} nhóm."
    except Exception as e:
        return f"❌ Lỗi autosend all on: {e}"


def autosend_all_off():
    try:
        with open("modules/cache/sendtask_autosend.json", "w") as f:
            json.dump({"groups": []}, f)
        return "🛑 Đã tắt autosend toàn bộ nhóm."
    except Exception as e:
        return f"❌ Lỗi autosend all off: {e}"
