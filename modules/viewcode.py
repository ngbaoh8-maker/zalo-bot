...
des = {
    'version': "1.0.1",
    'credits': "ngbao",
    'description': "Xem code (render ra ảnh, có fallback font an toàn)",
...
def load_font_safe(path, size):
    """
    Load font an toàn:
    - Nếu path tồn tại -> dùng truetype
    - Nếu không -> dùng font mặc định
    """
    try:
...
def create_image_from_code(
    code,
    command_name,
    font_path="modules/cache/font/NotoSans-Bold.ttf"
):
    fontcre = "modules/cache/font/UTM-AvoBold.ttf"
    fontlenh = "modules/cache/font/UTM-AvoItalic.ttf"
    ...
    temp_font = load_font_safe(font_path, 30)

    max_width = 0
    for line in code_lines:
        w = temp_draw.textlength(line, font=temp_font)
        if w > max_width:
            max_width = w

    max_width += 200
    img_width = max(3200, int(max_width))
    img_height = int(line_offset + len(code_lines) * line_height + 100)
    ...
    header_font = load_font_safe(font_path, 38)
    mitai_text = "Duy Khang Project"
    ...
    draw.text(
        (mitai_text_x, mitai_text_y),
        mitai_text,
        font=header_font,
        fill=(100, 100, 100)
    )

    # 3 DOT (giống cửa sổ code)
    dot_radius_outer = 14
    dot_radius_inner = 10
    dot_spacing = 25
    dots_x_start = 40
    dot_positions = [
        (dots_x_start, 35),
        (dots_x_start + 2 * dot_radius_outer + dot_spacing, 35),
        (dots_x_start + 4 * dot_radius_outer + 2 * dot_spacing, 35)
    ]
    ...
    for pos, border_color in zip(dot_positions, dot_colors):
        draw.ellipse(
            [pos[0] - dot_radius_outer, pos[1] - dot_radius_outer,
             pos[0] + dot_radius_outer, pos[1] + dot_radius_outer],
            fill=border_color
        )
        draw.ellipse(
            [pos[0] - dot_radius_inner, pos[1] - dot_radius_inner,
             pos[0] + dot_radius_inner, pos[1] + dot_radius_inner],
            fill=inner_dot_color
        )

    # TAB TÊN FILE
    command_font = load_font_safe(fontlenh, 28)
    command_text = f"{command_name}.py"
    command_text_width = temp_draw.textlength(command_text, font=command_font)

    python_logo_width = 26
    tab_padding = 25
    tab_width = int(command_text_width + python_logo_width + tab_padding * 3)
    tab_height = 60
    tab_color = background_color
    corner_radius = 18
    tab_x = dots_x_start + 4 * dot_radius_outer + 2 * dot_spacing + 50
    ...
    if os.path.exists(python_logo_path):
        try:
            ...
            python_logo = python_logo.resize((26, 26))
            background.paste(python_logo, (tab_x + tab_padding, tab_y + 10), python_logo)
        except Exception:
            pass

    draw.text(
        (tab_x + python_logo_width + tab_padding * 2, tab_y + 10),
        command_text,
        font=command_font,
        fill=(255, 255, 255)
    )
    ...
    code_font = load_font_safe(font_path, 28)
    line_font = load_font_safe(font_path, 28)
    ...
    border_size = 20
    background = ImageOps.expand(background, border=border_size, fill=border_color)

    image_path = "modules/cache/anh.png"
    background.save(image_path)
    return image_path, background.width, background.height
...
def get_color_for_token_type(span):
    color_map = {
        'k': (0, 255, 255),    # keyword
        'n': (255, 232, 255),  # name
        's': (255, 255, 107),  # string
        'c': (173, 216, 230),  # comment
        'o': (225, 225, 225),  # operator
        'p': (0, 255, 0),      # punctuation
        'm': (233, 51, 35),    # number
    }
...
def handle_viewcode_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if command_content is None:
        response_message = f"Lệnh '{command_name}' không được tìm thấy trong các commands."
        message_to_send = Message(text=response_message)
        client.replyMessage(message_to_send, message_object, thread_id, thread_type)
        return
