...
ACCOUNT_FILE = "modules/data/freefire.txt"
IMAGE_FILE = "modules/data/freefire/freefire.jpg"
DATA_FILE = "modules/data/freefire/freefire_data.json"
...
def handle_lienquan_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if sub == "set":
        if not is_admin(author_id):
            _reply_styled(client, "🚫 Bạn không có quyền cập nhật danh sách tài khoản Free Fire.", message_object, thread_id, thread_type)
            return
        ...
        try:
            ...
            _reply_styled(client, "✅ Đã cập nhật danh sách tài khoản Free Fire thành công!", message_object, thread_id, thread_type)
        except Exception as e:
    ...
    if sub == "setimg":
        if not is_admin(author_id):
            _reply_styled(client, "🚫 Bạn không có quyền thay ảnh minh họa Free Fire", message_object, thread_id, thread_type)
            return
        ...
        if _download_image(media_url, IMAGE_FILE):
            data["image_path"] = IMAGE_FILE
            _save_data(data)
            _reply_styled(client, "✅ Ảnh minh họa Free Fire đã được cập nhật thành công!", message_object, thread_id, thread_type)
        else:
    ...
    if sub is None or sub.isdigit():
        if not accounts:
            _reply_styled(client, "⚠️ File `modules/data/freefire.txt` hiện chưa có tài khoản nào.", message_object, thread_id, thread_type)
            return
        ...
        msg = "🎮 𝐓𝐚̀𝐢 𝐊𝐡𝐨𝐚̉𝐧 Free Fire 𝐜𝐮̉𝐚 𝐛𝐚̣𝐧:\n───────────────────\n"
        msg += "\n".join(selected)
        ...
        icons = ["đó e", "FF free kìa", "lượn đê ", "log đc chưa", "đm k làm đòi có ăn", "🤫", "🐧", "🥀"]
    ...
    _reply_styled(client,
        "⚠️ Sai cú pháp.\n"
        "• accff → nhận 1 tài khoản ngẫu nhiên\n"
        "• accff <số lượng> → nhận nhiều tài khoản\n"
        "• accff set → reply danh sách để cập nhật file\n"
        "• accff seting → reply ảnh để đổi ảnh minh họa",
