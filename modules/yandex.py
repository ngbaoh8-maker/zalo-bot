...
def handle_send_accounts_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    try:
        ...
        try:
            ...
            client.replyMessage(Message(text="Vui lòng cung cấp số link yandex."), message_object, thread_id, thread_type,ttl=20000)
            return

        # Đọc tài khoản từ file
        accounts = read_accounts_from_file('yandex.txt')
        if not accounts:
            client.replyMessage(Message(text="Không thể lấy link."), message_object, thread_id, thread_type,ttl=10000)
            return

        # Kiểm tra nếu số lượng yêu cầu vượt quá số lượng tài khoản có sẵn
        if num_accounts > len(accounts):
            client.replyMessage(Message(text=f"Chỉ có {len(accounts)} link có sẵn."), message_object, thread_id, thread_type,ttl=120000)
            return
