...
class ZaloAPI(object):
                ...
                if results.get("error_code") == 0:
                    ...
                    if results.get("fileId") and results["fileId"] != "-1":
                        ...
                        try:
                            ...
                            print(f"\nTimeout waiting for websocket: {e}")
                            if str(results["fileId"]) in self.uploadCallbacks:
                                del self.uploadCallbacks[str(results["fileId"])]
                            return None
    ...
    def upgradeCommunity(self, groupId, language="vi"):
        """Upgrade group to community.

        Args:
            groupId (int | str): Group ID to upgrade
            language (str): Language code (default: vi)

        Returns:
            dict: Upgrade result data if success
        ...
        with connect(url, additional_headers=headers) as ws:
            ...
            while not self._condition.is_set():
                try:
                    ...
                    try:
                        def find_file_info(d):
                            if isinstance(d, dict):
                                if 'fileId' in d and 'fileUrl' in d:
                                    fid = str(d['fileId'])
                                    if hasattr(self, 'uploadCallbacks') and fid in self.uploadCallbacks:
                                        self.uploadCallbacks[fid](d)
                                for k, v in d.items():
                                    find_file_info(v)
                            elif isinstance(d, list):
                                for item in d:
                                    find_file_info(item)
                        if hasattr(self, 'uploadCallbacks'):
                            find_file_info(parsedData)
                    except Exception:
                        pass
