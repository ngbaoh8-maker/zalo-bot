...
class ZaloAPI(object):
    ...
    def getAddFriendsList(self):
        """Lấy danh sách kết bạn
        
        Returns:
            object: Dữ liệu cuộc trò chuyện đã lưu trữ
        ...
        response = self._get("https://tt-friend-wpa.chat.zalo.me/api/friend/recommendsv2/list", params=params)
        data = response.json()
        results = data.get("data") if data.get("error_code") == 0 else None
    ...
    def setMute(self, thread_id, thread_type, duration=-1, is_mute=True):
        """Mute/Unmute a conversation thread.
        
        Args:
            thread_id (int | str): The User/Group ID to mute/unmute.
            thread_type (ThreadType): The type of thread (ThreadType.USER or ThreadType.GROUP).
            duration (int or str): The duration in seconds to mute for.
                - Use -1 for "Forever".
                - Use "until8am" to mute until 8 AM the next day.
                - Any other integer for a specific duration in seconds.
                This parameter is ignored when `is_mute` is False.
            is_mute (bool): True to mute the thread, False to unmute it.

        Returns:
            dict: A dictionary containing the API response on success.
            
        Raises:
            ZaloAPIException: If the API request fails.
        """
        ...
        if is_mute:
            if isinstance(duration, str) and duration.lower() == "until8am":
                now = datetime.now()
                next_8am = now.replace(hour=8, minute=0, second=0, microsecond=0)
                if now >= next_8am:
                    next_8am += timedelta(days=1)
                mute_duration = int((next_8am - now).total_seconds())
            else:
                try:
                    mute_duration = int(duration)
                except (ValueError, TypeError):
                    raise ZaloUserError("Invalid duration format. Must be an integer or 'until8am'.")
        ...
        params_to_encode = {
            ...
            "muteType": 1 if thread_type == ThreadType.USER else 2,
            "imei": self._imei,
        }

        payload = {
            "params": self._encode(params_to_encode)
        }
        ...
        if results:
            ...
            results = results.get("data") if results.get("data") else results
            if results is None:
                return {"status": "success", "message": "Mute status updated successfully."}
            
            if isinstance(results, str):
                try:
                    results = json.loads(results)
                except json.JSONDecodeError:
                    results = {"status": "success", "raw_data": results}
            
            return results
        
        error_code = data.get("error_code")
        error_message = data.get("error_message") or data.get("data")
        raise ZaloAPIException(f"Error #{error_code} when setting mute status: {error_message}")
        
    def muteGroup(self, groupId, mute=True, mute_until=None):
        """Mute or unmute a group.

        Args:
            groupId (int | str): The ID of the group
            mute (bool): True to mute, False to unmute
            mute_until (int, optional): Unix timestamp to mute until. Only used when mute=True

        Returns:
            dict: Success message or error info

        Raises:
            ZaloAPIException: If request failed at API level
        """
        payload = {
            "groupId": str(groupId),
        ...
        if mute:
            payload["muteUntil"] = mute_until if mute_until is not None else -1
        else:
            payload["muteUntil"] = 0
        ...
        response = self._post("https://tt-group-wpa.chat.zalo.me/api/group/updatesetting", params=params)
        data = response.json()
        results = data.get("data") if data.get("error_code") == 0 else None
        if results:
            results = self._decode(results)
            if results.get("error_code") == 0:
                ...
                return {"success": True, "message": f"Group {groupId} {action} successfully"}
            else:
                return {"error_code": results.get("error_code"), "error_message": results.get("error_message")}
        error_code = data.get("error_code")
        error_message = data.get("error_message") or data.get("data")
        raise ZaloAPIException(f"Error #{error_code} when muting group {groupId}: {error_message}")
    ...
    def createReminder(
        ...
        """Tạo một nhắc hẹn (Reminder) trong nhóm.
        
        Args:
            title (str): Tiêu đề nhắc hẹn
            groupId (int | str): ID của nhóm
            startTime (int): Thời gian nhắc hẹn (milliseconds). Mặc định là thời gian hiện tại.
            emoji (str): Emoji hiển thị (mặc định "⏰")
            repeat (int): Chế độ lặp lại:
                0 = Không lặp (None)
                1 = Hàng ngày (Daily)
                2 = Hàng tuần (Weekly)
                3 = Hàng tháng (Monthly)
            color (int): Mã màu (mặc định -16245706)
            
        Returns:
            object: `Group` reminder data
            dict: A dictionary containing error responses
    ...
    def createUserReminder(
        ...
        """Tạo một nhắc hẹn (Reminder) với người dùng (chat 1-1).
        
        Args:
            title (str): Tiêu đề nhắc hẹn
            userId (int | str): ID của người dùng
            startTime (int): Thời gian nhắc hẹn (milliseconds). Mặc định là thời gian hiện tại.
            emoji (str): Emoji hiển thị (mặc định "⏰")
            repeat (int): Chế độ lặp lại:
                0 = Không lặp (None)
        ...
        payload["params"] = self._encode(payload["params"])
        response = self._post("https://groupboard-wpa.chat.zalo.me/api/board/oneone/create", params=params, data=payload)
        data = response.json()
        results = data.get("data") if data.get("error_code") == 0 else None
        if results:
            results = self._decode(results)
    ...
    def sendLocation(self, latitude, longitude, thread_id, thread_type, title=None, address=None, ttl=0):
        """Send location message to a User/Group.
        ...
        Args:
            ...
            thread_id (int | str): User/Group ID to send to
            thread_type (ThreadType): ThreadType.USER, ThreadType.GROUP
            title (str): Title/name for the location (optional)
            address (str): Address description (optional)
        ...
        payload = {
            "params": {
                ...
                "msgInfo": {
                    "lat": float(latitude),
                    "lon": float(longitude),
                    "title": str(location_title),
                    "desc": str(location_desc)
                }
            }
        }
