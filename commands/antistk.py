...
class AntiStickerHandler:
    ...
    def handle_antistk_command(self, message_text, message_object, thread_id, thread_type, author_id):
        ...
        if action == "on":
            if self.is_enabled(thread_id_str):
                ...
                self.save_settings()
                rest_text = f"Đã bật Anti-Sticker. Vi phạm {self.kick_threshold} lần/15s sẽ bị chặn. 🛡️"
                reactions = ["✅", "🛡️", "🎨"]
        else: # action == "off"
    ...
    def check_and_delete_sticker(self, message_object, thread_id, thread_type, author_id):
        ...
        now = time.time()
        if thread_id not in self.sticker_violations: self.sticker_violations[thread_id] = {}
        user_violations = self.sticker_violations[thread_id].get(author_id, {'count': 0, 'first_violation_time': now})
        ...
        try:
            dm_text = (
                f"👑 ANTISTK 👑\n"
                f"🔧 Gì vậy người đẹp ai cho mà gửi stk???\n"
                f"➜ Ng dùng : @{author_id}\n"
                f"💢 Djt con mẹ mày </> Tân Xuân Hoàng đã k cho gửi stk r!\n"
            )
        ...
        if count >= self.kick_threshold:
            try:
                ...
                rest_text = f"📣{tag_author} đã bị chặn khỏi nhóm do spam sticker liên tục ({count}/{self.kick_threshold} sticker)."
        ...
        if msg:
            ...
            styles = MultiMsgStyle([
                MessageStyle(offset=len("➜ "), length=len("[ANTI-STICKER]"), style="color", color="#db342e", auto_format=False),
                MessageStyle(offset=len("➜ "), length=len("[ANTI-STICKER]"), style="bold", auto_format=False)
            ])
