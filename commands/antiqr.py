...
class AntiQRHandler:
    def __init__(self, client):
        self.client = client
        self.settings_path = "data/antiqr_settings.json"
    ...
    def handle_antiqr_command(
        ...
        if action not in ("on", "off"):
            ...
            msg = (
                f"{name}\n➜ 🚦Hướng dẫn: {PREFIX}antiqr <on/off>\n"
        ...
        if action == "on":
            ...
            text = "Đã bật Anti-QR (chặn link / QR)."
        else:
    ...
    # ================= DETECT =================
    def is_qr_message(self, message_object):
    ...
    def check_and_delete_qr(
        ...
        if not self.is_qr_message(message_object):
            return False
        ...
        if count >= self.kick_threshold:
            try:
                ...
                text = f"🚫 {name} đã bị kick do gửi link/QR nhiều lần."
            except Exception:
                text = f"⚠️ Không thể kick {name}."
        else:
            text = (
                f"⛔ Cấm gửi link / QR trong nhóm!\n"
