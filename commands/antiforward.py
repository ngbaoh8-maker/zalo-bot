...
class AntiForwardHandler:
    def __init__(self, client):
        self.client = client
        self.settings_path = "data/antiforward_settings.json"
    ...
    # ================= COMMAND =================
    def handle_antiforward_command(
        self,
        message_text,
        message_object,
        thread_id,
        ...
        if action not in ("on", "off"):
            ...
            msg = (
                f"{name}\n➜ 🚦Hướng dẫn: {PREFIX}antiforward <on/off>\n"
        ...
        if action == "on":
            ...
            text = "Đã bật Anti-Forward (cấm chuyển tiếp tin nhắn)."
        else:
            self.enabled_groups.pop(tid, None)
            text = "Đã tắt Anti-Forward."
    ...
    # ================= DETECT =================
    def is_forward_message(self, message_object, author_id):
        quote = getattr(message_object, "quote", None)
        if not quote:
            return False

        from_id = getattr(quote, "fromId", None)
        if from_id and str(from_id) != str(author_id):
            return True

        return False

    # ================= HANDLE =================
    def check_and_delete_forward(
        self,
        message_object,
        thread_id,
        thread_type,
        ...
        if not self.is_forward_message(message_object, author_id):
            return False
        ...
        if count >= self.kick_threshold:
            try:
                ...
                text = f"🚫 {name} đã bị kick do chuyển tiếp tin nhắn nhiều lần."
            except Exception:
                text = f"⚠️ Không thể kick {name}."
        else:
            text = (
                f"⛔ Cấm chuyển tiếp tin nhắn!\n"
